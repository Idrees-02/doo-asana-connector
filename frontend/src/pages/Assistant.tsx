 ...
 * The assistant.
 *
 * A conversational surface over the same connector core the API and MCP use.
 * Reads run automatically; a write comes back as a proposal the user has to
 * approve here, and only then is it executed through the ordinary action route
 * with `approved: true`.
 *
 * That gate is the whole design. The connector cannot delete a task or a
 * comment, so a model that could write unsupervised would be able to make
 * messes nobody can clean up — and Asana data flowing back into the model is
 * exactly the shape of a prompt-injection attack.
 */
...
import { Button, PageHeader, Panel, PanelHeader, StatusPill } from '@/components/ui';
...
export function Assistant() {
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-3xl flex-col">
      ...
      {disabled && (
        <Panel className="mb-3 p-4 text-sm text-(--color-ink-muted)">
          <div className="flex items-center gap-2">
            <AlertTriangle className="size-4 text-(--color-warning)" aria-hidden="true" />
            <span>
      ...
      <Panel className="flex min-h-0 flex-1 flex-col">
        <PanelHeader
          title="Conversation"
          description={
            status.data?.model === null || status.data?.model === undefined
              ? 'Connector actions, in plain language.'
              : `${status.data.model} · connector actions, in plain language.`
          }
        />
